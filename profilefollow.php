...
<html>
    ...
    <body>
        ...
        if (time() - intval($_GET["confirm"]) < 0) { // Check to see if the confirmation timestamp is in the future.
            echo "<p class=\"warning\">Warning: The confirmation timestamp is in the future. This should never happen. If you clicked an external link to get here, it's possible someone is trying to manipulate you into following this profile.</p>";
        } else if (time() - intval($_GET["confirm"]) < 10) { // Check to see if the confirmation timestamp is within the last 20 seconds.
            $instantiate_database[$username]["following"][$selected_profile] = array();
            save_database($instantiate_database);
            echo "<p>Successfully followed <b>" . $selected_profile . "</b></p>";
            header("Location: ./profileview.php?profile=" . $selected_profile);
        } else {
            echo "<p>Are you sure you would like to follow user <b>" . $selected_profile . "</b>?</p>";
            echo "<a class=\"button\" href=\"./profilefollow.php?profile=$selected_profile&confirm=" . time() . "\">Confirm</a>";
        }
        ?>
    </body>
</html>
