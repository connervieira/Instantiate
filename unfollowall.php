...
<html>
    ...
    <body>
        ...
        if (time() - intval($_GET["confirm"]) < 0) { // Check to see if the confirmation timestamp is in the future.
            echo "<p class=\"warning\">Warning: The confirmation timestamp is in the future. This should never happen. If you clicked an external link to get here, it's possible someone is trying to manipulate you into unfollowing all profiles.</p>";
        } else if (time() - intval($_GET["confirm"]) < 10) { // Check to see if the confirmation timestamp is within the last 20 seconds.
            $instantiate_database[$username]["following"] = array();
            save_database($instantiate_database); // Save the instantiate database.
            echo "<p>Successfully unfollowed all profiles</p>";
            header("Location: ./account.php");
        } else {
            echo "<p>Are you sure you would like unfollow all users known to this instance?</b>?</p>";
            echo "<a class=\"button\" href=\"./unfollowall.php?confirm=" . time() . "\">Confirm</a>";
        }
        ?>
    </body>
</html>
